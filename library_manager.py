...
if "search_resukts" not in st.session_state:
    st.session_state.search_results = []
if "book added" not in st.sesssion_state:
    st.session_state.book_added = False
if "book removed" not in st.sesssion_state:
...
def load_library():
    try:
        if os.path.exists("library.jason"):
...
def create_visulizations(stats):
    ...
    if stats["genres"]:
        ...
        fig_genres = px.bar(
            ...
            y='Count',
            color= 'Count',
            color_continious_scale= px.colors.sequential.Blues   
        )
...
lottie_book = load_lottieurl("https://assests0.lottiefiles.com/temp/1f20_akAfIn.json")
if lottie_book:
    with st.sidebar:
...
if new_options == "View Library":
    st.session_state.current_view = "library"
elif new_options == "Add Book":
    st.session_state.current_view = "add"
elif new_options == "Search Books":
    st.session_state.current_view = "search"
elif new_options == "Library Statistics":
...
if st.session_state.current_view == "add":
    ...
    with st.form(key='add_book_form'):
        ...
        if st.session_state.book_added:
            st.markdown("<div class= 'sucess-messsage'> Book added sucessfully!</div>", unsafe_allow_html=True)
            st.balloons()
            st.session_state.book_added = False
        ...
        for i , book in enumerate(st.session_state.library):
            with cols[i % 2]:
                st.markdown(f"""<div class = 'book-card'>
                            ...
                            <p><span class='{"read-badge" if book["read_status"] else"unread-status"}'>{
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.book_removed:
            st.markdown("<div class= 'sucess-messsage'> Book removed sucessfully!</div>", unsafe_allow_html=True)
            st.session_state.book_removed = False
